...
struct CursorHookConfigWriterTests {
    ...
    @Test func writeHookConfigSkipsGitTrackedFile() throws {
        let worktree = try makeTempDir()
        defer { try? FileManager.default.removeItem(at: worktree) }
        // Real git repo with a *committed* `.cursor/hooks.json`.
        func git(_ args: [String]) {
            let p = Process()
            p.executableURL = URL(fileURLWithPath: "/usr/bin/env")
            p.arguments = ["git", "-C", worktree.path] + args
        ...
        try FileManager.default.createDirectory(at: cursorDir, withIntermediateDirectories: true)
        let hooksPath = cursorDir.appendingPathComponent("hooks.json")
        let userContent: [String: Any] = ["version": 1, "hooks": [
            "beforeShellExecution": [["hooks": [["type": "command", "command": "/team/guard"]]]]]]
        try JSONSerialization.data(withJSONObject: userContent).write(to: hooksPath)
        git(["add", ".cursor/hooks.json"])
        let before = try Data(contentsOf: hooksPath)

        // Writing into a committed file must be refused (no Crow groups added).
        try CursorHookConfigWriter().writeHookConfig(
            worktreePath: worktree.path, sessionID: UUID(), crowPath: "/bin/crow")

        let after = try Data(contentsOf: hooksPath)
        #expect(after == before, "committed .cursor/hooks.json left untouched")
    }

    @Test func writeThenRemoveRoundTripsWithUnmanagedEvents() throws {
        let worktree = try makeTempDir()
        defer { try? FileManager.default.removeItem(at: worktree) }
        // User ships an unmanaged event; Crow writes its groups, then removes
        // them — the unmanaged event must round-trip untouched.
        let cursorDir = worktree.appendingPathComponent(".cursor")
        try FileManager.default.createDirectory(at: cursorDir, withIntermediateDirectories: true)
        let hooksPath = cursorDir.appendingPathComponent("hooks.json")
        let userGroup: [String: Any] = ["hooks": [["type": "command", "command": "/team/guard"]]]
        try JSONSerialization.data(withJSONObject: [
            "hooks": ["beforeShellExecution": [userGroup]]]).write(to: hooksPath)
        ...
        let hooks = try readHooks(hooksPath)
        #expect(hooks["beforeShellExecution"] != nil, "unmanaged event survives the round-trip")
        for (cursorKey, _) in [("stop", ""), ("preToolUse", "")] {
            #expect(hooks[cursorKey] == nil, "\(cursorKey) Crow groups removed")
        }
        let cmd = ((hooks["beforeShellExecution"] as! [[String: Any]]).first!["hooks"] as! [[String: Any]])
            .first!["command"] as! String
        #expect(cmd == "/team/guard")
    }

    @Test func writeHookConfigLeavesUnparseableFileUntouched() throws {
        // #829 Yellow 1: an unparseable .cursor/hooks.json (torn write / bad
        // hand-edit) must NOT be overwritten wholesale — that would drop the
        // user's own hook groups.
        let worktree = try makeTempDir()
        defer { try? FileManager.default.removeItem(at: worktree) }
        let cursorDir = worktree.appendingPathComponent(".cursor")
        try FileManager.default.createDirectory(at: cursorDir, withIntermediateDirectories: true)
        let hooksPath = cursorDir.appendingPathComponent("hooks.json")
        let torn = "{ \"hooks\": { \"preToolUse\": [ { \"hooks\": [ { \"command\": \"my-audit.sh\""
        try torn.write(to: hooksPath, atomically: true, encoding: .utf8)

        try CursorHookConfigWriter().writeHookConfig(
            worktreePath: worktree.path, sessionID: UUID(), crowPath: "/bin/crow")

        let after = try String(contentsOf: hooksPath, encoding: .utf8)
        #expect(after == torn, "unparseable hooks file left untouched")
        #expect(after.contains("my-audit.sh"))
    }
}
