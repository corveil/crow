...
public final class IssueTracker {
    ...
    /// "nothing happened" line, so at one line per 60s poll it doesn't rotate
    /// the interesting entries out of the log (review #787).
    ...
    private func reevaluateAutoMergeAfterLabel(session: Session, prURL: String) async {
        ...
        let result = await boardPoller.fetchStalePRStates(urls: [prURL], viewerLogin: "")
    ...
    private func prHasCrowAuthoredCommit(pr: ViewerPR, backend: CodeBackend) async -> Bool {
        ...
        attribution.recordPRAttribution(pr: pr, commits: commits)
