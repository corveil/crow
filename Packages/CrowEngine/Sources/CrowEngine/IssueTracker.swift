...
public final class IssueTracker {
    ...
    public func addMergeLabel(sessionID: UUID) async throws -> String? {
        ...
        do {
            ...
            // `ci:full` is a different action, applied by the approver's
            // `crow-review-pr` skill when `--approve` posts on a PR that
            // already has a `CI Gate` check (CROW-1306). This path only
            // records the author's merge intent.
