...
///
/// Facade (CROW-1275): protocol methods, `capabilities`, `shellRunner`,
/// `host` / `GITLAB_HOST`, and the `glab` argv wrappers. REST JSON parsers
/// live in `GitLabTaskBackend+Parsing.swift` as `extension GitLabTaskBackend`
/// so `@testable` tests keep `GitLabTaskBackend.parseIssues` /
/// `splitTotalHeader` and so `listAssigned` keeps calling the same statics.
///
