...
(async () => {
  ...
  check('Ticket shows no dropdown', !window.document.querySelector('.text-prompt-backdrop'));
  const ticketCall = calls.find((c) => c.method === 'todo-ticket');
  const ticketParams = ticketCall && ticketCall.params;
  check('todo-ticket receives only todo_id',
