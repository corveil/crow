...
| Problem                                                  | Solution                                                                 |
| -------------------------------------------------------- | ------------------------------------------------------------------------ |
| `crow` CLI: "Connection refused"                         | A server must be listening on `~/.local/share/crow/crow.sock`. That's the `crowd` daemon (`make daemon-run`, or run `crowd` with defaults). A "connection refused" (vs "no such file") means the socket file is stale — start `crowd` and it reclaims it. As of #234, `crow hook-event` is a silent no-op when nothing is listening, so non-Crow `claude` sessions no longer log noise. |
