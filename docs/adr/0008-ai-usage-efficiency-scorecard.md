...
| 3 | Persist a per-session compaction counter | Increment on `PostCompact` (completed compactions only — a failed/aborted compaction isn't graded waste) in the existing hook handler; persist with the session analytics snapshot from ticket 2, not on `PersistedHookState` (its documented contract is the color-driving UI subset and should stay that way). *Implemented (#704, completed in CROW-983):* the counter and the snapshot persist path landed with #704, but the increment was never called from the `hook-event` handler — `PostCompact` fell through to the state signal source's `default`, leaving the deduction inert on real data. CROW-983 wired it; see the Manager metering addendum below for the effect on existing grades. | v1 (depends on 2) |
...
| 6 | Rework / merge-rate metrics | Revert detection, merged-vs-closed PR rate, post-merge-fix tracking. *Implemented (#694):* all three attach to the #693 attribution records as optional fields and read out per session per window via `PRAttributionRepository.reworkMetrics` (`SessionReworkMetrics`: merged/closed counts, `mergeRate` — nil when no PRs resolved, revert + post-merge-fix counts). Reverts match git's `This reverts commit <sha>` line against stored branch/squash-merge SHAs, observed via attributed-PR commit fetches plus a throttled default-branch scan; a post-merge fix is another attributed PR, first seen after the merge, itself merged within 48h, touching ≥1 shared file, and not the revert (full heuristic documented on `PostMergeFixRecord`). Capture-only — grading/weighting stays with the v2 score (follow-up 11); the churn hint remains informational. | v2 |
| 7 | Per-turn analytics reader | Model turns from `telemetry.db` raw rows instead of `promptCount` proxies; enables within-session context-trend detection. *Implemented (#695):* `TelemetryDatabase.turnAnalytics(for:)` (surfaced via `TelemetryService.turnAnalytics(for:)`) reconstructs one `TurnAnalytics` record per `user_prompt` event by segmenting stored token/cost delta rows on their timestamps; per-field sums across turns match the session aggregate. Empty when raw rows aged out of retention or telemetry is off — callers fall back to the `promptCount` average. `EfficiencyGrading.perTurnInputTrendSlope` exposes the rising-vs-flat signal (least-squares, nil below 3 turns); pure reader — the grade stays on the per-session average until a follow-up wires the trend in and picks its threshold. | — |
...
### Backfill and Manager-usage addendum (implemented in #745)

The snapshot pipeline gained a recovery path and the Manager session gained visibility, with these decisions:

- **Backfill from telemetry.db**: snapshots were only ever written one-shot at a live `.completed`/`.archived` transition, so sessions recorded before snapshotting existed (or whose async write raced a quit) never surfaced — the scorecard sat empty while telemetry.db grew. `SessionService.backfillAnalyticsSnapshots()` now rebuilds missing snapshots at launch (before retention pruning, so about-to-age-out rows still count) and on a manual "Rebuild" action. Idempotent and conservative: existing snapshots are never overwritten, orphaned telemetry sessions (no Crow session record — status/`endedAt` would be fabricated) are skipped, non-terminal sessions wait for their completion write, and `endedAt` is taken from the session's `updatedAt` so historical sessions bucket into their true ISO week.
- **Manager usage is counted, but never graded**: a permanent Manager session can't reach a terminal status, so it gets `ManagerWeeklyUsage` rollups computed from time-windowed telemetry queries instead of completion snapshots — rendered as a separate "Manager Usage (ungraded)" card. This is consistent with this ADR's private-per-user posture (it is the user's own usage, on their own machine) and deliberately excluded from the grade, the shipped count, the combined score, and the baseline: an always-on orchestration session graded against task-session heuristics would be noise, and folding it in would dilute the coachable signal. Rollups are persisted merge-only, so weeks that age out of telemetry retention keep their values. *(The blanket "never graded" was narrowed by the CROW-983 addendum below: outcomes stay out, but the outcome-independent efficiency half is now graded.)*
- **Silent skips became diagnostics**: a completion whose analytics read empty (telemetry off, not restarted since enabling, or genuinely no data) now logs and increments a surfaced counter instead of silently preserving the empty scorecard; the empty state names every precondition (telemetry enabled + restart, non-Manager, terminal status, the grading prompt floor) and shows live capture status from telemetry.db.

### Manager metering addendum (implemented in CROW-983)

Manager sessions are now metered like work sessions, and the "ungraded" posture from #745 is split into its two real halves:

- **Efficiency is graded; outcomes are not.** ADR 0008's outcome flag is `status == .completed`, which a Manager can never reach — so the shipped count, cost-per-shipped, the combined score, and the trailing-4-week baseline stay closed to it. But the other four metrics (compactions per active hour, context pressure, cache hit ratio, API error rate) need no outcome at all, and they are the half that answers "is this session being run efficiently". Withholding them was hiding a signal the data already supported. The split needs no new grading code: `EfficiencyGrading.efficiencyInput(for:)` builds a `GradeInput` with `costContext` nil, and `costPerShippedDeduction` — the only outcome-touching deduction — returns nil without one. There is still exactly one `grade(_:)`.
- **Managers stay out of the graded aggregate, structurally.** `ScorecardModel` remains pure over completion snapshots; Manager rows join only at the DTO boundary, so `sessionsShipped`, `costPerShipped`, the combined score, and the baseline medians are byte-identical with and without Manager weeks present. Folding Manager cost into cost-per-shipped as overhead was considered and rejected here: a Manager *spawns* the sessions that ship, so charging its cost against their outcomes bills work it enabled but did not perform. That change would move a graded denominator and therefore requires an amendment to this ADR, not a code tweak.
- **Metering widened to the data already collected.** `ManagerWeeklyUsage` always persisted a full `SessionAnalytics`; `ManagerUsageWeekDTO` projected three fields of it. It now carries active time, cache hit ratio, input tokens per prompt, API error rate, compactions per active hour, tool calls, and commits alongside prompts/tokens/cost — a projection change, not new collection. The derived rates are read off the same `GradeInput` that produced the grade, so a chip cannot disagree with a deduction that cites it. Because `managerUsageWeekly` is merge-only and never pruned, the DTO now bounds each Manager to the window the refresh actually recomputes; previously the card grew a row a week forever.
- **Each Manager is attributed separately.** `crow create-manager` has always been able to make N Managers, but the rollup provider hardcoded the well-known primary UUID — so additional Managers were not merged into one bucket, they were **invisible**, despite emitting telemetry under their own session ids. Rollups are now keyed by week *and* session id and computed for every `.manager` session. Pre-CROW-983 rows carry no session id and resolve to the primary (the only session the old code queried); they are not rewritten in place, and where a legacy row and a recomputed row describe the same week, the recomputed one wins so the week is not rendered twice. `ManagerWeeklyUsage`'s new fields are `Optional` because `StoreData` decoding is all-or-nothing — a non-optional addition would fail the decode and reset the user's entire store.
- **Follow-up 3's compaction counter was finally wired.** The counter and its persist path shipped with #704, but nothing ever called `noteCompactionEvent`: `PostCompact` fell through to the state signal source's `default`, so the heaviest penalty in the rubric had been inert on real data since v1 — for work sessions too, not just Managers. The `hook-event` handler now counts it. Consequence: real work-session grades begin taking compaction deductions, so some will drop a letter; this is the metric behaving as designed for the first time, and it lands inside the tunable-priors/calibration framing above. A Manager's compactions are accumulated into the current ISO week as the event arrives rather than recomputed — telemetry.db has no compaction rows, and `SessionHookState.compactionCount` is a per-app-run running total with no week attribution, which a permanent session cannot be sliced out of after the fact.
