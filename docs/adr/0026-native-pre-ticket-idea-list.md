...
2. **Promotions reuse existing verbs.** `todo explore` is `create-manager` + `crow send` of an explore brief (pre-ticket sibling of `/crow-workspace --explore`). `todo ticket` types `/crow-create-ticket` into the primary Manager (the agent picks the repo; attach with `todo link`). `todo work` types `/crow-workspace` into the primary Manager. `todo talk` is sugar over `crow send` to the linked Manager.
