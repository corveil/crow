...
3. **Client — nothing to rewire.** `applySurfaceScrollback` and `fireScrollbackCapture` already gate on `agent_surface && uses_alternate_screen`, so the inline Claude inherits the Cursor treatment (unified 50k, CROW-606 replay, CROW-934 hydrate, CROW-1020 grabbable bar, #850 local wheel) for free. One consistency fix: `verifyScrollbackAfterAttach` (CROW-1027 post-attach one-screen heal) moves off `agent_surface`-alone onto the same `&& uses_alternate_screen` axis, so an inline agent (and Cursor) also gets that heal; a true alt-buffer agent is still skipped.
...
**Consequence — a startup transient, accepted.** An alt-buffer build (`2.1.233`) reports `uses_alternate_screen=false` for the ≤1 poll between its window's creation and its first `alternate_on=1`, so a starting Claude tab may show the 14px gutter briefly before it latches and the gutter vanishes. History is empty then, so nothing fossilizes — cosmetic only. The alternative (prior = capped) risks leaving a genuinely inline build capped-0 forever behind an unreliable "is it inline yet?" timeout, so the inline-until-proven-alt default is deliberate.
...
- **Runtime detection re-latches without a restart — unlike the frozen window option.** The `agent_surface` option is frozen at window birth (above), but `uses_alternate_screen` is re-derived from `alternate_on` on every `list-terminals` poll and latched (CROW-1023), so a build that enters or never enters the alt buffer is classified correctly within a poll of the window existing — no Recreate needed for the wheel/bar to be right. The one transient: an alt-buffer build shows a scrollbar gutter for ≤1 poll at startup before it latches (history is empty then, so cosmetic only).
...
- Code: `crow-tmux.conf` (alt-screen default, smcup/rmcup, mouse, history-limit); `TmuxController.setWindowOption` / `setSessionOption` / `listWindowScrollback` (per-window option + the `#{alternate-screen}` and `#{alternate_on}` read-back); `TmuxBackend.registerTerminal(agentSurface:usesAlternateScreen:)`, `enableAlternateScreen`, `isScrollbackDegraded(alternateScreenEnabled:)`, `agentSurfaceWindowIndices`, `updatedAltBufferLatch` / `altBufferWindowIndices` / `observedAltBufferWindows` (CROW-1023 sticky detection); `CodingAgent.usesAlternateScreen` (now the pre-window prior); `EngineRouter` `list-terminals` (`agent_surface`, `uses_alternate_screen` from the detected `altBuffer` set); `web/app.js` (`activeSurfaceIsAgent`, `activeSurfaceUsesAltScreen`, `applySurfaceScrollback`, `appOwnsScroll`, `swallowMouseMode`, `enableWheelScroll`, `enableTouchScroll`, `sendScrollToPTY`, `verifyScrollbackAfterAttach`); `TerminalCockpit.swift` / `TerminalWebSocket.swift` (CROW-606 replay).
- Tests: `ScrollbackHealthTests` (kind-aware policy truth table; leftover inline `history-limit 0` is degraded; CROW-1023 `updatedAltBufferLatch` sticky/prune/inline-never-latches truth table); `TmuxControllerTests.agentWindowOptsIntoAlternateScreenWithoutAffectingSiblings` / `sessionHistoryLimitSandwichClampsOnlyTheNewWindow` (real-tmux freeze); `TmuxBackendTests.inlineAgentSurfaceKeepsUnifiedHistoryWithoutAffectingSiblings`; `WebTerminalAssetTests` (client routing shape + alt-buffer-only scrollback cap); `web-tests/wheel-scroll.test.js` (jsdom wheel + inline agent keeps `UNIFIED_SCROLLBACK`); `web-tests/terminal-reload.test.js` (CROW-1027/1023 post-attach heal: alt-buffer skips, inline heals).
