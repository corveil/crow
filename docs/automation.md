...
- The session's PR pill carries a tinted ⛙ chip — green (auto-merge enabled, waiting on GitHub), purple (Crow merged it directly), orange (will retry), red (permanently blocked), grey (the watcher is off) — with the full reason in its tooltip and `aria-label`.
