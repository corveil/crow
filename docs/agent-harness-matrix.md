...
- **Claude:** `--permission-mode auto` (`ClaudeLaunchArgs`), the same knob the
  Manager uses ([ADR 0004](adr/0004-manager-auto-permission-mode.md)). **No
  longer stall-free as of Claude Code ≥ 2.1.257** (CROW-1176): auto mode shows
  a one-time prompt before the first file Read outside the working directories
  (launch cwd + `--add-dir` + `permissions.additionalDirectories`). Crow work
  / job worktrees, review clones under `{devRoot}/crow-reviews/`, and the
  Manager `{devRoot}` each have a different outside-set — sibling checkouts,
  `$TMPDIR`, and `$HOME` (e.g. `gh` creds) are the likely first hits, especially
  on `.review`. There is **no honest pre-answer**: `blockReadsOutsideWorkingDirectories`
  only *blocks* those Reads; `--add-dir` of `$HOME` / `$TMPDIR` / sibling
  trees would be the too-permissive fake (`--add-dir` also loads skills); and
  `--dangerously-skip-permissions` / `bypassPermissions` would also drop the
  same-release Containment Escape rule (cloud metadata / egress evasion /
  cross-tenant — **brittle-reject**, not a Crow gap). Crow still emits
  `--permission-mode auto` and does not paper over the prompt. A live Crow-tmux
  PTY pass on ≥ 2.1.257 remains a human re-check (eval binary was 2.1.232).
  Claude Code **≥ 2.1.259** later added `--permission-prompts none` (deny
  anything that would prompt, while auto still classifies and Containment
  Escape still applies). That is CROW-1176's deny-not-allow sibling, **not**
  `--dangerously-skip-permissions`. It is **print-mode only** (CLI reference:
  "Set who answers permission prompts in print mode"; example
  `claude -p --permission-prompts none "query"`). Crow launches the
  interactive TUI in a tmux PTY, not `-p`, so the flag cannot convert a TUI
  stall into a denial. **Declined (CROW-1215)** — do not emit it. Even if it
  became TUI-legal, applying it on the Manager would deny `$HOME` / `$TMPDIR`
  Reads that today only stall. Re-check only if the flag becomes TUI-legal;
  then evaluate for `.job` only, never Manager.
...
- **Antigravity:** the `jira` MCP is **mirrored** into Antigravity's user-scope
  `<$GEMINI_CONFIG_HOME or ~/.gemini/config>/mcp_config.json`
  (`mcpServers.jira`) by `AntigravityMCPConfigWriter` (CROW-1207). Official
  `agy` docs name that file for global servers (workspace-local
  `.agents/mcp_config.json` is a different surface — Crow does not write it).
  Same launch-gated posture as Cursor: it runs when an Antigravity agent
  actually launches (worker auto-launch, Manager, handoff, brand-new-terminal
  paste), not at daemon boot, so a box that merely has `agy` on PATH never
  gets the token copy. Merge-preserving — a user-authored `jira` entry is
  never overwritten; a `0600` provenance sidecar lets Crow refresh or
  un-mirror its own write. Remote Claude `url` / Gemini-legacy `httpUrl`
  become Antigravity's `serverUrl`. The `AntigravityLauncher` prompt
  instructs the `jira_*` MCP tools when a Claude `jira` source is present,
  and keeps `acli` only as the no-MCP fallback. Review clones still strip
...
restored a committed layer from the head (#902 review r2/r3). CROW-960 re-strips
in the skill immediately after that checkout for the *current* session; this
launch-path strip remains the next-process backstop if the agent skips the step.
Because the strip is the *only* defense, it removes the whole plausibly-discovered
surface rather than the native dir alone — the same posture
`stripGrokConfigFromReviewClone` takes.
...
| Claude auto-mode extra-workdir Read prompt can stall unattended sessions; no honest pre-answer | Claude Code **≥ 2.1.257** (`permissions.blockReadsOutsideWorkingDirectories` is a *block*, not a skip) | `ClaudeLaunchArgs` | 2026-09-02 (CROW-1176) — changelog-specified for `--permission-mode auto`. Working dirs = launch cwd + `--add-dir` + `permissions.additionalDirectories`. Crow worktree / review clone / Manager `{devRoot}` each have a different outside-set (sibling checkouts, `$TMPDIR`, `$HOME` for `gh` creds). No per-launch allow; `--dangerously-skip-permissions` / `bypassPermissions` would also drop the same-release Containment Escape rule (brittle-reject). `skipAutoPermissionPrompt` is a different notice. Installed probe binary was **2.1.232** (below the pin) so a live Crow-tmux PTY pass on ≥ 2.1.257 remains a human re-check. `--permission-prompts none` (2.1.259) is **not** that allow: it is deny-not-allow and print-mode only — declined CROW-1215 (next row) |
| Claude `--permission-prompts none` is print-mode only — **declined** as a TUI stall close | Claude Code **≥ 2.1.259** (changelog: unattended headless hosts); CLI reference: "Set who answers permission prompts in print mode"; headless docs example `claude -p --permission-mode auto --permission-prompts none` | `ClaudeLaunchArgs` (deliberately omit) | 2026-09-09 (CROW-1215) — **declined**. Auto still classifies and Containment Escape still applies (not `--dangerously-skip-permissions`). Crow launches the interactive TUI in a tmux PTY (ADR 0001), not `-p`, so the flag cannot convert extra-workdir auto-mode Read stalls into denials. Manager deny-fail on `$HOME`/`$TMPDIR` Reads would be worse than a stall even if it became TUI-legal. Launch tests pin that no Claude path emits `--permission-prompts`. Re-check only if the flag becomes TUI-legal (interactive, no `-p`) *and* is documented as converting extra-workdir auto-mode Read prompts into denials without dropping Containment Escape — then evaluate for `.job` only, never Manager |
...
| Antigravity review-clone strip list exhaustiveness: is any project-scope config `agy` reads **outside** `.agents/` + `.gemini/` still uncovered? | `agy` **v1.1.7**; user-scope MCP is `<$GEMINI_CONFIG_HOME or ~/.gemini/config>/mcp_config.json` (`AntigravityMCPConfigWriter`, CROW-1207). `AntigravityLaunchArgs` notes approval posture is governed by `settings.json` modes | `stripAntigravityConfigFromReviewClone` | 2026-09-07 (CROW-1207) — the user-scope writer confirms official docs: global MCP is `mcp_config.json` under `GEMINI_CONFIG_HOME` (default `~/.gemini/config`), not a project file Crow writes. The strip still removes `.agents/` **and** `.gemini/` **defensively** (a project `.agents/mcp_config.json` or `.gemini/settings.json` could carry `mcpServers`/`always-proceed`). So the question remains "is the strip **list exhaustive**" — confirm no other project-scope surface on the manual pass before promoting out of Tier-2 |
| Antigravity hook re-read timing: does `agy` read `.agents/hooks.json` **once at process start**, or per-event? | `agy` **v1.1.7** — unverified; assumed start-only | `prepareWorktreeForAgentLaunch` (launch-time strip) + `crow-review-pr` Step 1 re-strip (CROW-960) | 2026-08-28 (CROW-960) — the launch-time strip still mitigates a hook restored *between* launches. The review skill (and the re-review prompt) now re-strips immediately after `gh pr checkout`, which closes the mid-session restore for the current process **if the agent follows Step 1**. If `agy` re-reads per-event, the remaining window is checkout→`rm`, not checkout→session-end. Confirm re-read timing on the manual pass before promoting out of Tier-2 |
