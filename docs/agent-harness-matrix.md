...
| Gateway env / trust seed / telemetry | ✅ Claude special-case | ⚠️ trust seed only (`--trust`, per-launch) | ⚠️ trust seed only (`[projects."…"]` in `config.toml`) | ❌ | ❌ |
...
| Cursor interactive `--trust` (workspace-trust seed) — reverses the earlier headless-only omission | Cursor CLI **2026.07.20** (changelog: interactive); verified `agent 2026.07.23` (`--help` drops "headless mode only") | `CursorLaunchArgs.trustSuffix` | 2026-07-28 — re-probe if `--help` ever restores the headless-only qualifier |
