...
- **`sessionEnv`** — optional `KEY: VALUE` map exported into every agent launched in this workspace. Read by the `/crow-workspace` skill's setup script, one `KEY=VALUE` per line — so neither a key nor a value may contain a newline, and `crow workspace` rejects one that does. Unlike `gateway`, these values are **not** treated as credentials: they are stored in plain `config.json` and are not stripped from the web Settings payload, so put tokens in a gateway header instead.
...
- **`defaultAgentKind`** — the agent used for newly created sessions when no override applies. One of `claude-code`, `codex`, `cursor`, `opencode`, `antigravity`. Defaults to `claude-code`.
