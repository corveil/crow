...
| Hook async | ❌ sync-only (pinned v0.139.0) | ✅ **from 0.148.0** (re-checked 2026-08-13) | Skipped through `0.147.0` stable; `discovery.rs` on `main` now computes `runs_async = async && event != SessionEnd` → `HookExecutionMode::Async`. Carve-out: async `SessionEnd` is kept but run synchronously with a warning — irrelevant to Crow, which registers no `SessionEnd` hook. | **Landed (CROW-999).** `CodexVersionProbe` reads `codex --version` once at boot and `CodexHookConfigWriter` emits `async` for `PostToolUse` only at `>= 0.148.0`. Fail-closed on every uncertain answer, and pre-releases of the pin are rejected because async landed at `0.148.0-alpha.9` — an earlier alpha would still skip the hooks. `PreToolUse` stays sync (#903 apply-order). |
...
**Closed since:**
- **Codex async hooks** (§2 #4 / §3b) — upstream shipped them in `0.148.0`; wired behind a version probe in [CROW-999](https://github.com/corveil/crow/issues/999). What remains is empirical: the async **timing** (state-card correctness for `.job`/`.work`, no double-count against the `notify` bridge) has no observed pass yet, because `0.148.0` is **still pre-release** — npm `dist-tags` on 2026-08-13 are `latest` = `0.147.0`, `alpha` = `0.148.0-alpha.14`. The gate rejects pre-releases, so it is inert on every stable install until the release lands; that first stable build is where the timing pass belongs.
