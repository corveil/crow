...
**Audited:** 2026-07-23. **Codex RC row re-probed:** 2026-08-13 against installed `codex-cli 0.141.0` ([CROW-1001](https://github.com/corveil/crow/issues/1001) — §2 #3 / §3b Remote control). **Codex hook-async row re-checked:** 2026-08-13 against upstream `main` ([CROW-999](https://github.com/corveil/crow/issues/999) — §2 #4 / §3b Hook async). **Cursor persist re-probed:** 2026-09-02 against installed `cursor-agent 2026.08.31-4057e58` ([CROW-1175](https://github.com/corveil/crow/issues/1175) — §3a Resume / Remote control). **Claude `--permission-prompts none` re-checked:** 2026-09-09 against official CLI reference + headless docs ([CROW-1215](https://github.com/corveil/crow/issues/1215) — Auto-permission; print-mode only, declined).
...
|---|---|---|---|---|
| Resume / continue | ❌ "no `--continue` in MVP" | ✅ | `agent --continue`, `agent --resume [chatId]`, `agent resume`, `agent ls` (landed [CLI Jan 16 2026](https://cursor.com/changelog/cli-jan-16-2026): `/list`→`/resume`) | On `.job`/`.review` restart, replace the bare `agent` fallback in `CursorAgent.autoLaunchCommand` with `agent --continue`, so re-open restores history instead of a cold TUI. **Landed #829.** Cursor CLI **2026.08.26** later added `agent persist` (keep running after disconnect). That is **not** a further `--continue` close: persist is Cursor's own tmux wrapper and **stays unwired** (CROW-1175 — see Persist row). |
| Persist (`agent persist`) | — (shipped after the 2026-07-23 audit) | ⚠️ upstream yes; **Crow declined** | `agent persist` / `list` / `attach` / `stop` / `--resume`; `/detach` in-session ([changelog 2026-08-26](https://cursor.com/docs/cli/changelog); min **≥ 2026.08.26**; probed `cursor-agent 2026.08.31-4057e58`) | **Declined ([CROW-1175](https://github.com/corveil/crow/issues/1175)).** Persist is TTY-detach via a second tmux server (`tmux -L cursor-agent -f /dev/null`, `new-session -d`, `destroy-unattached off`); `/detach` is `tmux detach-client`. If `$TMUX` is already set, persist **forwards argv to a normal `agent` launch** — Crow always launches inside tmux (ADR 0001), so persist as a Crow launch command would never persist. After detach there is no TUI for `crow send` to paste into; persist also requires interactive stdin/stdout. `persist list`/`stop` are machine-global on that named server (names `cursor-<slug>-<hash>-<n>-<rand>`, not Crow UUIDs) — same defect class as Codex's app-server daemon. Crow's tmux pane already *is* the "survive TTY disconnect" layer; `crow recreate-terminal` keeps `--continue`. Re-check only if persist starts *from inside* `$TMUX` without nesting, addresses sessions by an id Crow can scope, and stays paste-drivable while detached. |
...
| Remote control | ⚠️ faked via `crow send` paste | ⚠️ (no dedicated local RC socket) | — | No change recommended; `crow send` paste remains the simplest driver. `supportsRemoteControl=true` stays correct. **`agent persist` is not native RC** and was evaluated separately (CROW-1175, Persist row above) — TTY-detach via a second tmux server, no-ops in `$TMUX`, still needs an attached pane for paste. Same reject class as native `codex remote-control`. |
