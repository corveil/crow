...
- **A role also rejects an agent that can't run that kind of session.** Today that is only `--review antigravity`: Antigravity has no review dispatch, so those sessions would be created and then never launch an agent. This mirrors `crow handoff-agent`, which already refuses the same pair — it would be incoherent to refuse the handoff but let you configure it. The gate applies to the role flags only, not `--default`: validating the *resolved* outcome would make a pre-existing default fail every later write, including patches to unrelated roles. A default that resolves review to a review-incapable agent surfaces at launch time instead, where Crow writes an explanatory line into the terminal.
...
- **`--session-env` is one variable per entry.** A newline in a key or value is rejected — the `/crow-workspace` setup script reads the map as one `KEY=VALUE` per line, so a newline would smuggle in a second variable. This is enforced server-side, not just by the CLI.
