...
create_session() {
  ...
  # Step 4: Add ticket link (if URL provided). GitHub/GitLab chips read
  # `Issue #N`; Jira (non-numeric) stays `Issue` (CROW-1167).
  if [[ -n "$TICKET_URL" ]]; then
    log "Adding ticket link..."
    local ticket_label="Issue"
    [[ "$TICKET_NUMBER" =~ ^[0-9]+$ ]] && ticket_label="Issue #$TICKET_NUMBER"
