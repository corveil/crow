...
- #816 — The web session-menu actions are now CLI verbs: `crow mark-in-review`, `complete-session`, `set-session-active`, `mark-issue-done`, `add-merge-label`. Preconditions the browser enforced by hiding menu items (linked ticket, linked PR, not a Manager) moved server-side, so `mark-issue-done` / `add-merge-label` report a real error instead of returning `{"ok":true}` for a call that did nothing. The three status transitions now route through `SessionService`, restoring the analytics snapshot a completed session is meant to record.
